...
export default {
    ...
    watch: {
        ...
        data(to) {
            if (Object.keys(to).length > 0) {
                let { id, store_id, name, mobile_number, email, user_type } = to;
                this.formData = { ...this.formData, id, store_id, name, mobile_number, email, user_type };
            }
        },
        dialog(to) {
            if (!to) {
    ...
    methods: {
        getDataFromAPI () {
            this.$store.dispatch('getStores', { selection: true })
                .then(res => {
                    console.log(res)
                })
                .catch(error => {
